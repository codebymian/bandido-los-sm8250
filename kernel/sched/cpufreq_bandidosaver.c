				     ...
				     unsigned int next_freq)
{
	...
	if (next_freq > sg_policy->next_freq) {
	    bool dex_active = secdp_check_dex_mode();
	    if ((lcd_is_on || dex_active) && delta_ns < sg_policy->up_rate_delay_ns)
	        return true;

	    if (!lcd_is_on && !dex_active && delta_ns < 20000000)
	        return true;
	}
			      ...
			      unsigned long *max)
{
	...
	bool is_rtg_boost = sugov_rtg_boost_active(sg_cpu);
...
static struct cpufreq_governor bandidosaver_gov;
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	case 4:
		...
		tunables->hispeed_freq = 1478400;
		tunables->up_rate_limit_us = 64000;
		break;
	case 7:
		tunables->rtg_boost_freq = DEFAULT_CPU7_RTG_BOOST_FREQ;
		tunables->hispeed_freq = 1401600;
		tunables->up_rate_limit_us = 128000;
		break;
	}
...
{
	return cpufreq_register_governor(&bandidosaver_gov);
}
