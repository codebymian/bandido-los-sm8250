					    ...
					    struct list_head *page_list)
{
	...
	nr_reclaimed = shrink_page_list(&clean_pages, zone->zone_pgdat, &sc,
			TTU_IGNORE_ACCESS, &stat, true, false);
					...
					struct vm_area_struct *vma)
{
	...
	nr_reclaimed = shrink_page_list(page_list, NULL, &sc,
			TTU_IGNORE_ACCESS, NULL, true, false);
...
#ifdef CONFIG_LRU_GEN_WALK_MMU
struct lru_gen_mn {
	struct mmu_notifier mn;
	struct mm_struct *mm;
};

static int lru_gen_mmu_notifier_invalidate_range_start(struct mmu_notifier *mn,
						       const struct mmu_notifier_range *range)
{
	struct lru_gen_mn *lgmn = container_of(mn, struct lru_gen_mn, mn);

	if (range->end > range->start)
		WRITE_ONCE(lgmn->mm->lru_gen.invalidated, true);

	return 0;
}

static const struct mmu_notifier_ops lru_gen_mmu_notifier_ops = {
	.invalidate_range_start = lru_gen_mmu_notifier_invalidate_range_start,
};
#endif
...
#ifdef CONFIG_LRU_GEN_WALK_MMU
	mm->lru_gen.notifier = NULL;
	mm->lru_gen.invalidated = false;

	if (lru_gen_enabled()) {
		struct lru_gen_mn *lgmn = kzalloc(sizeof(*lgmn), GFP_KERNEL);

		if (lgmn) {
			lgmn->mm = mm;
			lgmn->mn.ops = &lru_gen_mmu_notifier_ops;
			if (mmu_notifier_register(&lgmn->mn, mm))
				kfree(lgmn);
			else
				mm->lru_gen.notifier = &lgmn->mn;
		}
	}
#endif
}

void lru_gen_exit_mm(struct mm_struct *mm)
{
#ifdef CONFIG_LRU_GEN_WALK_MMU
	if (mm->lru_gen.notifier) {
		struct mmu_notifier *mn = mm->lru_gen.notifier;
		struct lru_gen_mn *lgmn = container_of(mn, struct lru_gen_mn, mn);
...
static void walk_mm(struct lruvec *lruvec, struct mm_struct *mm, struct lru_gen_mm_walk *walk)
{
	...
	do {
		...
		if (walk->next_addr > FIRST_USER_ADDRESS) {
#ifdef CONFIG_LRU_GEN_WALK_MMU
			if (READ_ONCE(mm->lru_gen.invalidated)) {
				walk->next_addr = FIRST_USER_ADDRESS;
				WRITE_ONCE(mm->lru_gen.invalidated, false);
				err = -EAGAIN;
			}
#endif
		}
		       ...
		       bool *swapped)
{
	...
	reclaimed = shrink_page_list(&list, pgdat, sc, 0, NULL, false, true);
